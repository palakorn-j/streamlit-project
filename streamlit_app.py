...
def validate_logiccondition_or_except_special(df_tests, df_sorts):
    ...
    for idx, val in df_sorts["LogicCondition"].items():
        if val not in allowed_special and val != "OR":
            errors.append(
                f"Row {idx + 1}: LogicCondition '{val}' is invalid; must be 'OR' or one of {allowed_special}.")
...
def check_failbranch_uniform(df_tests, df_sorts):
    ...
    for idx, val in df_tests["FailBranch"].items():
        ...
        if val != common_value:
            item = df_tests.at[idx, "ItemName"] if "ItemName" in df_tests.columns else ""
            errors.append(
                f"Row {idx}: FailBranch ({val}) does not match common value ({common_value}) for item '{item}'")
...
VALIDATION_RULES = {
    ...
    "Some Test Item is not specified Fail Branch": check_failbranch_vs_sequence,
...
tab1, tab2, tab3 = st.tabs(["📁 Single File Validation", "🗂️ Multiple File Validation", "⚠️ MSS Validation"])
...
# ------------------------------------------------------
# TAB 3: MSS Specification Compare
# ------------------------------------------------------
with tab3:
    st.header("MSS Specification Compare")
    ...
    if uploaded_spec_file:
        ...
        if df_tests is not None and not df_tests.empty:
            ...
            mask = spec_draft["ItemName"].isin(
                ['BVCBO', 'BVCBR', 'BVCBS', 'BVCEO', 'BVCER', 'BVCES', 'BVDG1O', 'BVDG2O', 'BVDGO',
